              ...
              >
                ...
                >
                  <div class="flex items-center gap-2.5">
                    ...
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 leading-relaxed">{{ month.label }}</span>
                    <span class="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">({{ month.posts.length }})</span>
                  </div>
                  <Icon
                    icon="lucide:chevron-down"
                ...
                <div class="accordion-body" :class="{ expanded: isMonthExpanded(month.key) }">
                  <div>
                    ...
                    >
                      <span class="text-xs font-mono text-gray-400 dark:text-gray-500 shrink-0 flex items-center whitespace-nowrap leading-normal">
                        <Icon icon="lucide:calendar-days" class="w-3 h-3 mr-1 text-[#7CB342]" />
                        {{ formatDate(post.date, 'short') }}
                      </span>
                      <h4 class="flex-1 text-sm font-medium text-gray-700 dark:text-gray-300 group-hover/item:text-[#7CB342] transition-colors truncate leading-relaxed">
                        {{ post.title }}
                      </h4>
                      <div class="hidden sm:flex gap-1.5 shrink-0">
                        <span
                          ...
                          :key="tag"
                          class="text-xs px-2.5 py-1 rounded-full bg-gray-100 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 leading-normal"
                        >
                          {{ tag }}
                        </span>
                      </div>
