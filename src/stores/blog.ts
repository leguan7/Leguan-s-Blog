...
export const useBlogStore = defineStore('blog', () => {
  ...
  // View counts (busuanzi results cached in localStorage for persistence)
  const VIEW_CACHE_KEY = 'blog_busuanzi_views'
  const viewCounts = ref<Record<string, number>>({})

  // Load cached counts from localStorage on init
  try {
    const raw = localStorage.getItem(VIEW_CACHE_KEY)
    if (raw) viewCounts.value = JSON.parse(raw)
  } catch { /* ignore */ }

  function getViewCount(slug: string): number {
    return viewCounts.value[slug] || 0
  }

  function setViewCount(slug: string, count: number) {
    viewCounts.value[slug] = count
    // Persist to localStorage so counts survive page refresh
    try {
      localStorage.setItem(VIEW_CACHE_KEY, JSON.stringify(viewCounts.value))
    } catch { /* ignore */ }
  }
